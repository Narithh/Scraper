...
class DDGScraper:
    ...
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        """
        self.args = args
        # set instance verbosity (no module-level VERBOSE used anymore)

        self.scraped_results: List[tuple] = []
        self.skipped_urls: List[str] = []
        self.browser = None
        self.page = None
        # verbosity flag used by instance methods
        self.verbose = bool(getattr(args, "verbatim", False))

    @staticmethod
    def parse_args() -> argparse.Namespace:
        """Parse CLI arguments (static so it can be called before instantiation)."""
    ...
    def vprint(self, *args, **kwargs):
        """Instance-level verbose print."""
        if self.verbose:
            print(*args, **kwargs)

    def is_captcha_page(self, page) -> bool:
        ...
        if any(keyword in body_text for keyword in self.CAPTCHA_KEYWORDS):
            return True
    ...
    def raise_captcha(self, context: str, hint: Optional[str] = None) -> None:
    ...
    def ensure_not_captcha(self, page, context: str) -> None:
        """Guard clause that exits early if the current page looks like a captcha."""

        if self.is_captcha_page(page):
            self.raise_captcha(context)

    def human_delay(self, min_ms: int = 200, max_ms: int = 600) -> None:
    ...
    def extract_main_text(self, html: str) -> Optional[str]:
    ...
    def fetch_search_results(self, page, query: str, num_sites: int) -> List[str]:
        """Perform the DuckDuckGo query and return absolute URLs for the top results."""

        search_url = f"https://duckduckgo.com/?q={quote_plus(query)}"
        self.vprint(f"Searching DuckDuckGo for: {query}")
        ...
        try:
            ...
            self.ensure_not_captcha(page, "DuckDuckGo search page")
            self.human_delay()
            ...
            try:
                ...
                self.ensure_not_captcha(page, "DuckDuckGo HTML search page")
                self.human_delay()
        ...
        if not results:
            ...
            self.vprint(f"Found {len(results)} result(s).")

        return results

    def scrape_page(self, page, url: str, max_words: int) -> Optional[str]:
        ...
        self.vprint(f"Opening {url}")
        html = None
        for attempt in range(2):
            try:
                page.goto(url, wait_until="domcontentloaded")
                # immediate captcha check after navigation
                self.ensure_not_captcha(page, url)
                self.human_delay()
                html = page.content()
                self.ensure_not_captcha(page, url)
                break
        ...
        if not html:
            self.vprint(f"No HTML captured from {url}; skipping.")
            return None

        text = self.extract_main_text(html)
        if not text:
            self.vprint(f"Could not extract readable content from {url}")
            return None

        return self.truncate_words(text, max_words)

    def write_markdown(self, output_path: Path, results: List[tuple], append: bool) -> None:
    ...
    def run(self) -> int:
        ...
        try:
            with sync_playwright() as p:
                ...
                try:
                    urls = self.fetch_search_results(self.page, self.args.query, self.args.num_sites)
                    if not urls:
                        self._close_browser()
                        return 1

                    for url in urls:
                        text = self.scrape_page(self.page, url, self.args.max_words)
                        if text:
                    ...
                    self.write_markdown(self.args.output, self.scraped_results, self.args.append)
                    self.vprint(f"Saved {len(self.scraped_results)} entr(ies) to {self.args.output}")
                    if self.skipped_urls:
                        ...
                        for s in self.skipped_urls:
                            self.vprint(f"  - {s}", file=sys.stderr)
